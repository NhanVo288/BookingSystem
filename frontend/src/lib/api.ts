    ...
    refreshPromise = fetch(`${API_URL}/auth/refresh-access-token`, {
      ...
      .then(async (r) => {
        ...
        if (data.refreshToken)
          localStorage.setItem("roomly.refreshToken", data.refreshToken);
        return true;
      })
      .finally(() => {
        refreshPromise = null;
  ...
  if (!response.ok) {
    const body =
      typeof data === "object" && data
        ...
        : undefined;
    throw new ApiError(
      String(body?.detail || body?.title || data || `Lỗi ${response.status}`),
      response.status,
      body,
    );
  }
...
export const body = (value: unknown): RequestInit => ({
